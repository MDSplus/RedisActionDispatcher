...
class ActionDispatcher:
            ...
            if d.isOn():
                ...
                try:
                    ...
                    if not isinstance(when, MDSplus.Scalar): #if it is a dependent action
                        self.dependencies[treeShot][d.getNid()] = when.getNid()
                    ...
                    self.red.hset('ACTION_INFO:'+tree.name+':'+str(tree.shot), tree.getNode(actNid).getFullPath(), 'NOT_DISPATCHED')
    ...
    def performSequenceStep(self, tree, phase):
        ...
        for ident in self.seqActions[treeShot][phase].keys():
            if len(self.pendingActions[ident]) == 0:
                ...
                if self.currSeqNumbers[ident] <= self.endSeqNumber:
                    ...
                    for actNid in self.seqActions[treeShot][phase][ident][self.currSeqNumbers[ident]]:
                        self.pendingActions[ident].append(actNid)
                        self.red.lpush('ACTION_SERVER_TODO:'+ident, 
                            tree.name+'+'+str(tree.shot)+'+'+tree.getNode(actNid).getFullPath()+'+'+str(self.timeouts[treeShot][actNid]))
                        print('Dispatching action '+tree.getNode(actNid).getFullPath()+'   Tree: '+tree.name+'  Shot: '+str(tree.shot))
                        self.actionDispatchStatus[treeShot][actNid] = self.DISPATCHED
                        self.red.hset('ACTION_INFO:'+tree.name+':'+str(tree.shot), tree.getNode(actNid).getFullPath(), 'DISPATCHED')
                    self.red.publish('ACTION_SERVER_PUBSUB:'+ident, 'DO')


    def doPhase(self, tree, phase):
        seqIdents = self.seqActions[tree.name+str(tree.shot)][phase].keys()
        minSeqNumber = sys.maxsize
        maxSeqNumber = 0
        for seqIdent in seqIdents:
            for seqNum in self.seqActions[tree.name+str(tree.shot)][phase][seqIdent]:
    ...
    def handleNotifications(self):
        while True:
            ...
            try:
                ...
                continue
            path = parts[3]
            print('Action '+parts[3]+ '   terminated')
...
#handle sequence
            ...
            if not ident in self.pendingActions.keys():
                print('Internal error: unextected ident: '+ident)
                self.updateMutex.release()
                continue
            if actionNid in self.pendingActions[ident]:
                self.pendingActions[ident].remove(actionNid)
            if len(self.pendingActions[ident]) == 0:
            ...
            if actionNid in self.depAffected[treeShot].keys():
                for depNid in self.depAffected[treeShot][actionNid]:
                    if self.checkDispatch(tree, depNid):
                        ...
                        self.red.lpush('ACTION_SERVER_TODO:'+ident, 
                            treeName+'+'+str(shot)+'+'+tree.getNode(depNid).getFullPath()+'+'+str(self.timeouts[treeShot][depNid]))
                        print('Dispatching action '+tree.getNode(depNid).getFullPath()+'   Tree: '+tree.name+'  Shot: '+str(tree.shot))
                        self.red.hset('ACTION_INFO:'+treeName+':'+str(shot), tree.getNode(depNid).getFullPath(), 'DISPATCHED')
                        #in ogni caso
                        self.red.publish('ACTION_SERVER_PUBSUB:'+ident, 'DO')
...
# return True if the dispatching condition is satisfied
    ...
    def checkDone(self, when, tree):
        ...
        if isinstance(when, MDSplus.Compound):
            opcode = when.getOpcode()
            if opcode == MDSplus.treeDEPENDENCY_AND or opcode ==  MDSplus.treeLOGICAL_AND:
                return self.checkDone(when.getArgumentAt(0)) and self.checkDone(when.getArgumentAt(1))
            if opcode ==  MDSplus.treeDEPENDENCY_OR or opcode ==  MDSplus.treeLOGICAL_OR:
